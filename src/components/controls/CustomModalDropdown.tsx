...
const CustomModalDropdown = <T,>({
    ...
    return (
      <TouchableOpacity
        ...
        style={[
          styles.touchable,
          {
            backgroundColor: showDropdown ? 'white' : 'rgba(244, 244, 244, 1)',
          },
        ]}>
        <View
          style={{
          ...
          <MyText
            ...
            numberOfLines={2}
            style={[
              {fontFamily: getFontFamily('500'), marginRight: sizes[10]},
              textStyle,
            ]}>
            {value}
          </MyText>
          <DesignIcon name={'arrow-down'} fill={text} size={sizes[7]} />
        </View>
      </TouchableOpacity>
    );
  };
                  ...
                  <View
                    style={[
                      styles.bottom,
                      {
                        ...
                        borderTopWidth: extraBorder ? 1 : 0,
                      },
                    ]}>
  ...
  const _calcPosition = () => {
    ...
    const bottomSpace =
      windowHeight - _buttonFrame.current.y - _buttonFrame.current.h;
    const rightSpace = windowWidth - _buttonFrame.current.x;
    const showInBottom =
      bottomSpace >= dropdownHeight || bottomSpace >= _buttonFrame.current.y;
  ...
  const _renderItem = (item: ListRenderItemInfo<IOption<T>>) => {
    ...
    return (
      <TouchableOpacity
        style={[styles.renderItem, styleRowComponent]}
        onPress={handlePress}>
        {renderRow ? (
          renderRow(
            {
